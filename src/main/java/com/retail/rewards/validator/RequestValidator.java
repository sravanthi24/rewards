...
public class RequestValidator {
	...
	public void validatePhoneNumber(String number, List<Error> errorList) {
		...
		if (StringUtils.isBlank(number)) {
			...
			errorList.add(error);
		}else if(number.length()<3 && number.length()>15){
			error = new Error();
			error.setErrorCode(ApiErrorCode.INVALID_PHONENUMBER_LENGTH.getCode());
			error.setErrorDescription(ApiErrorCode.INVALID_PHONENUMBER_LENGTH.getDescription());
			errorList.add(error);
		}
	}

	public void validateTransactionAmount(BigDecimal amount, List<Error> errorList) {
		Error error = null;
		if (amount == null) {
			error = new Error();
